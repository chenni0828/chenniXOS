...
echo]
set key="HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Search" /v AllowSearchToUse位置
choice /c:yn /n /m "Would you like web search to use your 位置 for results? [Y/N] "
if %errorlevel%==1 reg delete %key% /f > nul 2>&1
if %errorlevel%==2 reg add %key% /t REG_DWORD /d 0 /f > nul

:: Enable 搜索索引 to prevent a visual bug
...
:: Main settings
call "%windir%\chenniXModules\Scripts\settingsPages.cmd" /unhide search-permissions /silent
(
...
:searchIndexBug
echo]
echo On Windows 11, having 搜索索引 disabled causes a graphical bug in web search.
choice /c:yn /n /m "Would you like to enable 搜索索引 to fix it? [Y/N] "
if %errorlevel%==1 call "%windir%\chenniXDesktop\3.配置\搜索索引\Enable 搜索索引.cmd" /silent & set ____restart=true
exit /b