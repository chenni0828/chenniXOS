...
echo          recommended to use 'Set Windows Default 缓解措施.cmd' instead.
echo]
...
:: Set all bits to 1 (enable all 缓解措施)
for /l %%a in (0,1,9) do (
...
:: Enable file system 缓解措施
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager" /v "ProtectionMode" /t REG_DWORD /d "1" /f > nul

:: Enable for Hyper-V
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Virtualization" /v "MinVmVersionForCpuBased缓解措施" /t REG_SZ /d "1.0" /f > nul
