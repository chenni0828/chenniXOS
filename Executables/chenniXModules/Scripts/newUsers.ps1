...
if ([System.Environment]::OSVersion.Version.Build -ge 22000) {
    & "$chenniXDesktop\2.系统配置\界面\右键菜单\Windows 11\Old Context Menu (default).cmd" /silent
    & "$chenniXDesktop\2.系统配置\界面\文件资源管理器\图库\Disable Gallery (default).cmd" /silent
...
& "$chenniXDesktop\2.系统配置\配置\文件共享\网络导航窗格\Disable Network Navigation Pane (default).cmd" /silent
