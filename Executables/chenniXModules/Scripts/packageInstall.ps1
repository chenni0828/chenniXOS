...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\chenniXModules\Packages",
...
$safeModePackageList = "$sys32\safeModePackagesToInstall.chenniXmodule"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgTitle = 'chenniXFailedComponentMsgBox'
			$failedMsgArgs = "/c title Finalizing Installation - chenniX & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the chenniX team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($UninstallPackages) {
	$installedPackages = @()
	$notMatchedPackages = $UninstallPackages
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to chenniX." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to chenniX." -ForegroundColor Red
...
是否需要 chenniX 重启到安全模式并重试？此过程不会花费太多时间。

请注意，如果您选择"否"，且之前选择了禁用 Windows Defender，它可能仍保持启用状态。不过，您可以随时在 chenniX 文件夹中尝试再次禁用。
"@

	if ((Read-MessageBox -Title "chenniX - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
# https://github.com/chenniX-OS/chenniX/issues/1103
function MakeRepairSource {
	$version = '38655.38527.65535.65535'
	$srcPath = "%SystemRoot%\chenniXModules\Packages\WinSxS"
