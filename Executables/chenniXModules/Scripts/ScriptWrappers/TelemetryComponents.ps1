...
$packageInstall = "$windir\chenniXModules\Scripts\packageInstall.ps1"
...
$package = "*Z-chenniX-NoTelemetry-Package*"
...
function Menu {
	...
	Write-Host "This script adds or removes chenniX' NoTelemetry package, which removes certain telemetry components in Windows.`n" -ForegroundColor Cyan

	Write-BulletPoint @"
Removing the package restores the telemetry components, which can aid in troubleshooting. If
   resolving the issue involves removing the package, please report this as an issue to chenniX.`n
"@
	Write-BulletPoint @"
chenniX retains policies that should disable telemetry even after the package removal. These policies
